...
def get_stock_names(code, retry=3):
    ...
    if "证券简称更名历史：" in stock_info:
        stock_info["证券简称更名历史："]=stock_info["证券简称更名历史："].replace("*","").replace("（","").replace("）"," ").replace("("," ").replace(")"," ").replace("  "," ").strip().split(' ')
    else:
        stock_info=None
    return stock_info
...
if __name__=="__main__":
    ...
    if Need_Craw:
        ...
        Stock_Names=[n.replace(" ","").replace("*","").strip() for n in data['name']]
        ...
        for code,name in zip(Stock_Codes,Stock_Names):
            ...
            if stock_info is not None:
                ...
                stock_info['公司简称：']=stock_info['公司名称：'].replace("股份有限公司","").replace("有限责任公司","")
                with open(Dic_File,'a') as f:
                    ...
                    for n in stock_info['证券简称更名历史：']:
                        ...
                        if 'A' in n or 'B' in n or 'G' in n or 'N' in n:
                            f.write(n.replace('A','').replace('B','').replace('G','').replace('N','')+'\n')
